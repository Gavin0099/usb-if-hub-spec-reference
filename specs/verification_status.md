...
| Artifact type | Count | Status |
|---|---:|---|
| USB 3.x evidence packets | 48 | `evidence/entry_verification_packets/usb3/`；9 筆對應 SS hub descriptor (USB3-3A)，10 筆對應 SS hub class requests (USB3-3B)，15 筆對應 SS port status bits (USB3-3C)，6 筆對應 SS feature selectors (USB3-FS-2)，4 筆對應 SS wHubCharacteristics bit groups (USB3-WHC-1)，4 筆對應 SS hub interrupt endpoint (USB3-IEP-1) |

SS port status bit matrix 已完成 USB3-3C pilot：15 個 defined entries 升為 verified。SS feature selector matrix 已完成 USB3-FS-2 pilot：6 個 SS-only port feature selector entries 升為 verified（selector name/value/applicability/recipient identity only）；4 個 port status reserved boundary entries 維持 reviewed（永久邊界）。SS wHubCharacteristics bit matrix 已完成 USB3-WHC-1 pilot：4 個 defined bit groups 升為 verified（bit group name/value encoding identity only）；1 個 reserved boundary 維持 reviewed。SS hub interrupt endpoint matrix 已完成 USB3-IEP-1 pilot：4 個 descriptor 欄位全部升為 verified（field identity/constraint encoding only）。
...
## LTSSM-0 導向參考（非 governed surface）

Phase LTSSM-0 新增 `ss_ltssm.md`（ZH + EN），作為 LTSSM 狀態名稱、狀態群組與高層次轉換路徑的導向參考。這是 readability layer；**不是 governed surface 的新增**，不改變任何 governed matrix 統計數字。

**LTSSM-0 包含：**
- LTSSM 狀態群組概覽（7 個群組：Disabled/Detect、Inactive、Training、Active Link、Low Power、Recovery/Reset、Test/Special）
- 高層次轉換導向表（僅列常見後續狀態路徑）
- Hub `PORT_LINK_STATE` 與 LTSSM 狀態名稱的對應關係

**不包含（永久邊界）：**
- 完整的 normative LTSSM transition matrix
- LFPS 信號波形或時序參數
...
**Claim ceiling：** `claim_level: inferred`，`semantic_verification_claimed: false`，`governed_surface: false`。LTSSM-0 不擴展 USB 3.x 的 verified 或 reviewed entry 計數。

---

## USB 3.x Feature Selector Expansion（USB3-FS-2）

> 本節**獨立於上方 38/34/4 matrix-level closeout baseline**，不改變 USB 3.x closeout 數字。
> Verified gate：PARTIAL / allowlist-only（USB3-FS-2 pilot，全 6 筆已完成 verified promotion）。

Phase USB3-FS-2 完成第四張 USB 3.x governed matrix 的 verified promotion：
六筆 USB 2.0 feature selector namespace 中不存在的 SS-only port feature selector 全部升為 verified。
...
| **合計** | | | **6 verified / 0 reviewed** | gate：**PARTIAL（allowlist，全 6 筆）** |

Evidence packets（6 筆，`evidence/entry_verification_packets/usb3/`）：
...
**所有 SS feature selector entries 的 non-claims（固定邊界）：**
- 不宣告 U1/U2 power state 進入或離開行為。
- 不宣告 U1/U2 enable 或 BH reset 觸發的 LTSSM transition behavior。
- 不宣告 xHCI port power policy 或 xHCI warm reset behavior。
- 不宣告 U1/U2 timeout 編碼語意或 wValue 欄位行為。
- 不宣告 remote wake event routing、platform wake policy 或 OS 電源管理。
- 不宣告 BH/warm reset 時序、LFPS signaling 或 link recovery outcome。

## USB 3.x wHubCharacteristics Bit Matrix（USB3-WHC-1）

> Verified gate：PARTIAL / allowlist-only（USB3-WHC-1 pilot，4 筆 defined bit groups 已完成 verified promotion）。
> USB 3.x 無 TT；Port Indicators 位於 bit[5]（USB 2.0 為 bit[7]）。
...
| usb3_ss_whc_reserved_high | `wHubCharacteristics` | bits[15:6] | reviewed | reserved boundary (永久邊界) |
| **合計** | | | **4 verified / 1 reviewed** | gate：**PARTIAL（allowlist，4 筆）** |

Evidence packets（4 筆，`evidence/entry_verification_packets/usb3/ss_whc_*.yaml`）。

## USB 3.x Hub Interrupt Endpoint Matrix（USB3-IEP-1）

> Verified gate：PARTIAL / allowlist-only（USB3-IEP-1 pilot，全 4 筆已完成 verified promotion）。
> USB 3.x SuperSpeed bInterval 使用 2^(n-1)×125μs microframe 編碼（range 1–16）；無 FS/LS 的直接 ms 編碼。
...
| **合計** | | **4 verified / 0 reviewed** | gate：**PARTIAL（allowlist，4 筆）** |

Evidence packets（4 筆，`evidence/entry_verification_packets/usb3/ss_iep_*.yaml`）。
...
| 元件 | 路徑 | 角色 |
|---|---|---|
| Unified manifest | `exports/hub_governed_surface_manifest.yaml` | Governed truth index：15 張 tables 的 authority surface、claim ceiling、consumer usage contract |
| Fingerprint baseline | `evidence/table_fingerprint_baseline.jsonl` | 15 張 governed tables 的 content-hash baseline，用於 CI drift detection |
...
- specs/en/verification_status.md: English counterpart topic (en).
