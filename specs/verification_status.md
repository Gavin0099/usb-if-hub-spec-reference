...
> 本節**獨立於上方 38/34/4 matrix-level closeout baseline**，不改變 USB 3.x closeout 數字。Verified gate CLOSED。

Phase USB3-FS-1 新增第四張 USB 3.x governed matrix（expansion scaffold 階段）：
六筆 USB 2.0 feature selector namespace 中不存在的 SS-only port feature selector。
...
| **合計** | | | **0 verified / 6 reviewed** | gate：**CLOSED** |
...
- Verified promotion 需等 USB3-FS-2；manifest/baseline 納入需等 EXPORT-CONTRACT-1.1。
