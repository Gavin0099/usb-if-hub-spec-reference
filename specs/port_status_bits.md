...
目前只有下面列出的 11 個 live verified entries 完成 entry-level promotion。

## Live Verified Entries

目前有 11 個 live governed entries 已提升為 `verified`：
...
- live `verified` promotions：目前包含 `PORT_CONNECTION`、`PORT_ENABLE`、`PORT_SUSPEND`、`PORT_OVER_CURRENT`、`PORT_RESET`、`C_PORT_CONNECTION`、`C_PORT_ENABLE`、`HUB_LOCAL_POWER`、`HUB_OVER_CURRENT`、`C_HUB_LOCAL_POWER`、`C_HUB_OVER_CURRENT`
...
- 本頁不把 11 個 verified entries 擴張成整頁 verified。
