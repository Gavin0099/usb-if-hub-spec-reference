...
Only the 11 live verified entries listed below have completed entry-level promotion.

## Live Verified Entries

Eleven live governed entries are currently promoted to `verified`:
...
- live `verified` promotions, currently for `PORT_CONNECTION`, `PORT_ENABLE`, `PORT_SUSPEND`, `PORT_OVER_CURRENT`, `PORT_RESET`, `C_PORT_CONNECTION`, `C_PORT_ENABLE`, `HUB_LOCAL_POWER`, `HUB_OVER_CURRENT`, `C_HUB_LOCAL_POWER`, and `C_HUB_OVER_CURRENT`
...
- This page does not expand 11 verified entries into a claim that the whole page is verified.
