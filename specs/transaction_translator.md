...
| Request | bRequest | 設定摘要 |
|---|---:|---|
| `CLEAR_TT_BUFFER` | `0x08` | 清除 TT 緩衝狀態（僅 TT-capable HS hub） |
| `RESET_TT` | `0x09` | 重設 TT（僅 TT-capable HS hub） |
| `GET_TT_STATE` | `0x0A` | 取回 TT 診斷狀態（僅 TT-capable HS hub） |
| `STOP_TT` | `0x0B` | 停止 TT split-transaction 處理（僅 TT-capable HS hub） |

上述欄位摘要只做 request 對照用途；`wValue`、`wIndex`、`wLength` 的行為邊界仍依各自 matrix/頁面補充。
